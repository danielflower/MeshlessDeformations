/* This is LINPACK Library code which was obtained from here: 
...
//
  ...
  for ( j = 1; j <= n; j++ )
  {
    anorm = d_max ( anorm, dasum ( l, abd+is-1+(j-1)*lda, 1 ) );
    if ( ml + 1 < is )
    {
      is = is - 1;
    }
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( d_abs ( abd[m-1+(k-1)*lda] ) < d_abs ( ek - z[k-1] ) )
    {
      s = d_abs ( abd[m-1+(k-1)*lda] ) / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
    ...
    ju = i_min ( i_max ( ju, mu+ipvt[k-1] ), n );
    mm = m;
    ...
    if ( k+1 <= ju )
    {
      ...
      {
        mm = mm - 1;
        sm = sm + d_abs ( z[j-1] + wkm * abd[mm-1+(j-1)*lda] );
...
//
  for ( k = n; 1 <= k; k-- )
  {
    if ( d_abs ( abd[m-1+(k-1)*lda] ) < d_abs ( z[k-1] ) )
    {
      s = d_abs ( abd[m-1+(k-1)*lda] ) / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
{
  ...
  for ( i = 1; i <= n; i++ )
  {
    ...
    while ( d_abs ( det[0] ) < 1.0 )
    {
      det[0] = ten * det[0];
      det[1] = det[1] - 1.0;
    }

    while ( ten <= d_abs ( det[0] ) )
    {
      det[0] = det[0] / ten;
      det[1] = det[1] + 1.0;
    }
...
//
  ...
  for ( j = 1; j <= n; j++ )
  {
    anorm = d_max ( anorm, dasum ( n, a+0+(j-1)*lda, 1 ) );
  }
//
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( d_abs ( a[k-1+(k-1)*lda] ) < d_abs ( ek - z[k-1] ) )
    {
      s = d_abs ( a[k-1+(k-1)*lda] ) / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
    ...
    if ( k+1 <= n )
    {
      for ( j = k+1; j <= n; j++ )
      {
        sm = sm + d_abs ( z[j-1] + wkm * a[k-1+(j-1)*lda] );
...
//
  for ( k = n; 1 <= k; k-- )
  {
    if ( d_abs ( a[k-1+(k-1)*lda] ) < d_abs ( z[k-1] ) )
    {
      s = d_abs ( a[k-1+(k-1)*lda] ) / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
  for ( j = 1; j <= n; j++ )
  {
    ...
    {
      k = k + 1;
      z[k-1] = z[k-1] + d_abs ( abd[i-1+(j-1)*lda] );
    }
  }
  anorm = 0.0;
  for ( i = 1; i <= n; i++ )
  {
    anorm = d_max ( anorm, z[i-1] );
  }
//
//  Factor.
//
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( abd[m+(k-1)*lda] < d_abs ( ek - z[k-1] ) )
    {
      s = abd[m+(k-1)*lda] / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
    ...
    if ( k+1 <= j2 )
    {
      ...
      {
        i = i - 1;
        sm = sm + d_abs ( z[j-1] + wkm * abd[i-1+(j-1)*lda] );
...
//
  for ( k = n; 1 <= k; k-- )
  {
    if ( abd[m+(k-1)*lda] < d_abs ( z[k-1] ) )
    {
      s = abd[m+(k-1)*lda] / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
  ...
  for ( i = 1; i <= n; i++ )
  {
    det[0] = abd[m+(i-1)*lda] * abd[m+(i-1)*lda] * det[0];
...
//
  for ( j = 1; j <= n; j++ )
  {
    ...
    for ( i = 1; i <= j-1; i++ )
    {
      z[i-1] = z[i-1] + d_abs ( a[i-1+(j-1)*lda] );
    }
  }
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( a[k-1+(k-1)*lda] < d_abs ( ek - z[k-1] ) )
    {
      s = a[k-1+(k-1)*lda] / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
  for ( k = n; 1 <= k; k-- )
  {
    if ( a[k-1+(k-1)*lda] < d_abs ( z[k-1] ) )
    {
      s = a[k-1+(k-1)*lda] / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
  if ( job / 10 != 0 )
  {
    ...
    for ( i = 1; i <= n; i++ )
    {
      det[0] = a[i-1+(i-1)*lda] * a[i-1+(i-1)*lda] * det[0];
...
//
  ...
  for ( k = 1; k <= n; k++ )
  {
    ...
    if ( ap[kk-1] < d_abs ( ek - z[k-1] ) )
    {
      s = ap[kk-1] / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
  for ( k = n; 1 <= k; k-- )
  {
    if ( ap[kk-1] < d_abs ( z[k-1] ) )
    {
      s = ap[kk-1] / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
...
//
    ...
    if ( l != n )
    {
      ...
      if ( nrmxl != 0.0 )
      {
        ...
        {
          nrmxl = nrmxl * d_sign ( a[l-1+(l-1)*lda] );
        }
...
//
        for ( j = l + 1; j <= p; j++ )
        {
          ...
          if ( pl <= j && j <= pu )
          {
            if ( qraux[j-1] != 0.0 )
            {
              tt = 1.0 - pow ( d_abs ( a[l-1+(j-1)*lda] ) / qraux[j-1], 2 );
              tt = d_max ( tt, 0.0 );
              t = tt;
...
//
    else
    {
      ...
      for ( j = imaxp1; j <= k; j++ )
      {
        rowmax = d_max ( rowmax, d_abs ( a[imax-1+(j-1)*lda] ) );
      }

      if ( imax != 1 )
      {
        jmax = idamax ( imax-1, a+0+(imax-1)*lda, 1 );
        rowmax = d_max ( rowmax, d_abs ( a[jmax-1+(imax-1)*lda] ) );
      }

      if ( alpha * rowmax <= d_abs ( a[imax-1+(imax-1)*lda] ) )
      {
        kstep = 1;
        swap = true;
      }
...
//
  ...
  while ( k != 0 )
  {
    ...
    if ( ks != 2 )
    {
      if ( d_abs ( ap[kk-1] ) < d_abs ( z[k-1] ) )
      {
        s = d_abs ( ap[kk-1] ) / d_abs ( z[k-1] );
        for ( i = 1; i <= n; i++ )
        {
          z[i-1] = s * z[i-1];
        }
...
//
    km1 = k - 1;
    kk = ik + k;
    absakk = d_abs ( ap[kk-1] );
//
//  Determine the largest off-diagonal element in column K.
//
    imax = idamax ( k-1, ap+ik, 1 );
    imk = ik + imax;
    colmax = d_abs ( ap[imk-1] );
...
//
    else
    {
      ...
      for ( j = imaxp1; j <= k; j++ )
      {
        rowmax = d_max ( rowmax, d_abs ( ap[imj-1] ) );
        imj = imj + j;
      }
      ...
      if ( imax != 1 )
      {
        ...
        jmim = jmax + im;
        rowmax = d_max ( rowmax, d_abs ( ap[jmim-1] ) );
      }

      imim = imax + im;

      if ( alpha * rowmax <= d_abs ( ap[imim-1] ) )
      {
        kstep = 1;
        swap = true;
      }
...
void dsvdc ( double a[], int lda, int m, int n, double s[], double e[], 
  double u[], int ldu, double v[], int ldv, double work[], int job, int *info )
...
//
//    Output, double E[N], ordinarily contains zeros.  However see the
...
//
//    Output, int *INFO, status indicator.
...
//
    if ( l <= nct )
    {
      ...
      if ( s[l-1] != 0.0 )
      {
        if ( a[l-1+(l-1)*lda] != 0.0 )
        {
          s[l-1] = d_sign ( a[l-1+(l-1)*lda] ) * d_abs ( s[l-1] );
        }
...
//
    for ( ll = 1; ll <= mn; ll++ )
    {
      ...
      test = d_abs ( s[l-1] ) + d_abs ( s[l] );
      ztest = test + d_abs ( e[l-1] );
    ...
    else
    {
      ...
      for ( lls = l+1; lls <= mn+1; lls++ )
      {
        ...
        if ( ls != mn )
        {
          test = test + d_abs ( e[ls-1] );
        }

        if ( ls != l + 1 )
        {
          test = test + d_abs ( e[ls-2] );
        }

        ztest = test + d_abs ( s[ls-1] );
...
//  Calculate the shift.
//
      scale = d_max ( d_abs ( s[mn-1] ), 
              d_max ( d_abs ( s[mn-2] ), 
              d_max ( d_abs ( e[mn-2] ), 
              d_max ( d_abs ( s[l-1] ), d_abs ( e[l-1] ) ) ) ) );
...
//
  ...
  for ( j = 1; j <= n; j++ )
  {
    ...
    tnorm = d_max ( tnorm, dasum ( l, t+i1-1+(j-1)*ldt, 1 ) );
  }
//
...
//
  ...
  for ( kk = 1; kk <= n; kk++ )
  {
    ...
    if ( d_abs ( t[k-1+(k-1)*ldt] ) < d_abs ( ek - z[k-1] ) )
    {
      s = d_abs ( t[k-1+(k-1)*ldt] ) / d_abs ( ek - z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
    ...
    if ( kk != n )
    {
      ...
      for ( j = j1; j <= j2; j++ )
      {
        sm = sm + d_abs ( z[j-1] + wkm * t[k-1+(j-1)*ldt] );
...
//
  for ( kk = 1; kk <= n; kk++ )
  {
    ...
    if ( d_abs ( t[k-1+(k-1)*ldt] ) < d_abs ( z[k-1] ) )
    {
      s = d_abs ( t[k-1+(k-1)*ldt] ) / d_abs ( z[k-1] );
      for ( i = 1; i <= n; i++ )
      {
        z[i-1] = s * z[i-1];
      }
